...
- xcode-select symlinks (`/var/select/developer_dir`, `/var/db/xcode_select_link`, plus `/private/var/...` twins) granted as `literal` reads. `/usr/bin/git` is a libxcselect shim that resolves the active developer dir from these links before exec'ing the real git; without read access it concludes "no developer tools" and triggers the macOS Command Line Tools install dialog every run, even after CLT is installed. The literals expose only the symlink targets and dirent listings of `/var/select` and `/var/db` — not the contents of sibling system metadata files. When the active dir is under `/Library/Developer/CommandLineTools` (already granted as a subpath above) or when CLT is installed alongside Xcode (libxcselect falls back to CLT cleanly), no extra grant is emitted. For Xcode-only setups the `.app` bundle root is granted as a `subpath` so DVT* frameworks at sibling `<bundle>/Contents/{Frameworks,SharedFrameworks}` resolve. Selection logic lives in `sandbox.DetectXcodeReadSubpath`. The same set also includes `/var/select/sh` and `/private/var/select/sh` — the BSD-select symlink for `/bin/sh`, read by the spawn path when git invokes the shell for hooks, pager, aliases, and worktree rebuilds (`git reset --hard`, `git checkout`); without it git aborts with `Error opening /private/var/select/sh: Operation not permitted`
- Timezone data (`/var/db/timezone` plus `/private/var/...` twin) granted as `subpath` reads. `/etc/localtime` is a symlink into this tree (typically `-> /var/db/timezone/zoneinfo/America/New_York`); the `/etc` subpath grant covers reading the symlink itself, but resolving it lands on a denied target, so libc `localtime()` silently falls back to UTC inside the sandbox — producing wrong-zone timestamps in `git log`, npm output, Node `Date()`, Python `datetime.now()`, and similar. The data is root-owned and world-readable system zoneinfo (`tz`, `zoneinfo`, `icutz`); no user data is exposed
- Global git config — `~/.gitconfig` (literal) and `~/.config/git` (subpath, the XDG fallback used when `~/.gitconfig` is absent and the canonical home for `core.excludesfile` and `attributes`) granted as **read-only** when `Policy.DenyHomeGitconfig=false` (the default). The agent inherits the user's identity, signing prefs, and global ignore patterns without being able to mutate them. `~/.config/git/credentials` is denied as a Layer-3 mandatory home literal alongside `~/.git-credentials` — read-only access to a credential store still leaks the token. Set `DenyHomeGitconfig=true` for stricter sandboxes that must not leak global git settings; both legacy and XDG paths are then omitted
